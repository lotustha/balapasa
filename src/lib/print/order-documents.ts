...
// Builds the full printable HTML document for one or more orders. Shared by the
// admin print endpoint and the customer-scoped invoice route — callers are
// responsible for authorization before passing order ids in.
export async function renderPrintDocument(ids: string[], type = 'shipping'): Promise<string> {
