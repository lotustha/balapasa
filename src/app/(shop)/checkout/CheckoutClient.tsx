...
function weatherDelaySecs(w: WeatherD): number {
  ...
  let delay = 0
  if (id >= 200 && id < 300) delay = 60 * 60
  else if (id >= 300 && id < 400) delay = 5 * 60
  else if (id >= 500 && id < 600) delay = 30 * 60
  else if (id >= 600 && id < 700) delay = 90 * 60
  else if (id >= 700 && id < 800) delay = 20 * 60
  if (wind > 8) delay += 10 * 60
  return delay
}
...
function nepalTrafficDelaySecs(): number {
  ...
  if (hour >= 8  && hour < 10)  return 25 * 60
  if (hour >= 12 && hour < 13)  return 10 * 60
  if (hour >= 17 && hour < 20)  return 30 * 60
...
function savedToNepalAddress(addr: SavedAddress): NepalAddress {
  return {
...
export default function CheckoutClient({ user, initialAddresses }: CheckoutClientProps) {
  ...
  // Cart hydrates from localStorage in an effect — wait for mount before
  ...
  const [selectedSavedId, setSelectedSavedId] = useState<string | null>(defaultSaved?.id ?? null)
  const [isAddingNew, setIsAddingNew] = useState<boolean>(!defaultSaved)

  // "Save my info" — only relevant for guests
  const [saveInfo, setSaveInfo] = useState<boolean>(true)

  // Pre-fill from user + default saved address (if any)
  const [recipientName,  setName]  = useState(defaultSaved?.name  ?? user?.name  ?? '')
  const [recipientPhone, setPhone] = useState(defaultSaved?.phone ?? '')
  const [recipientEmail, setEmail] = useState(user?.email ?? '')
  ...
  async function fetchWeather(municipality: string, district: string) {
    ...
    } catch { /* weather is non-critical */ } finally {
  ...
  async function applyGiftCard() {
    const code = giftCard.trim().toUpperCase()
    if (!code) { setGiftCardError('Enter a gift card code'); return }
    setGiftCardValidating(true); setGiftCardError('')
    try {
      ...
      if (!res.ok) {
        ...
        setGiftCardBalance(0); setGiftCardApplied(false)
      } else {
        setGiftCardBalance(Number(data.balance) || 0)
  ...
  function removeGiftCard() { setGiftCard(''); setGiftCardApplied(false); setGiftCardBalance(0); setGiftCardError('') }
  ...
  useEffect(() => {
    if (defaultSaved && defaultSaved.province && defaultSaved.district && defaultSaved.municipality) {
      const addr = savedToNepalAddress(defaultSaved)
      maybeFetchCoverage(addr)
      fetchWeather(addr.municipality, addr.district)
    }
  ...
  function selectSavedAddress(addr: SavedAddress) {
    ...
    const nepal = savedToNepalAddress(addr)
    setAddress(nepal)
    if (nepal.province && nepal.district && nepal.municipality) {
      lastCoverageKey.current = ''
      maybeFetchCoverage(nepal)
      fetchWeather(nepal.municipality, nepal.district)
    } else {
      setOptions([]); setSelected(null)
    }
  }
  ...
  async function deleteSaved(id: string) {
    ...
    if (selectedSavedId === id) {
      ...
      if (next) selectSavedAddress(next)
      else startAddNew()
    }
  }
    ...
    try {
      ...
      const res = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          road: address.street,
          // Structured address fields for save-info / saved-address round-trip
          ...
          lat: typeof address.lat === 'number'
            ? address.lat
            : typeof (selectedOption?.meta as Record<string,unknown>)?.receiverLat === 'number'
            ? (selectedOption!.meta as Record<string,unknown>).receiverLat : undefined,
          lng: typeof address.lng === 'number'
            ? address.lng
            : typeof (selectedOption?.meta as Record<string,unknown>)?.receiverLng === 'number'
            ? (selectedOption!.meta as Record<string,unknown>).receiverLng : undefined,
          // Guest signup nudge
          createAccount: !user && saveInfo && !!recipientEmail.trim() ? true : undefined,
          // Logged-in: link order to this saved address (informational; orders API may ignore)
          savedAddressId: user && !isAddingNew ? selectedSavedId : undefined,
        }),
      })
      const data = await res.json()
      ...
      // Logged-in: persist a new/edited address now if user typed one in
      if (user && isAddingNew && address.province && address.district && address.municipality && address.street.trim() && address.tole.trim()) {
        try {
      ...
      // Redirect to the dedicated success route. Magic-link token (if any) is
      // included as a query param so the guest-signup card on the success page
      // can render the "claim your account" button.
      const orderId = data.orderId ?? data.order?.id ?? ''
      const claimParam = data.magicLinkToken ? `?claim=${encodeURIComponent(data.magicLinkToken)}` : ''
      router.push(orderId ? `/checkout/success/${orderId}${claimParam}` : '/checkout/success')
    } catch {
      setPlacing(false)
    }
  }

  // Suppress unused-var warning for cartSubtotal (kept for future use)
  void cartSubtotal
  ...
  if (isEmpty) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-12"
        ...
        <div className="max-w-md w-full bg-white/80 backdrop-blur rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/40 p-8 text-center">
          <div className="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-br from-indigo-100 to-pink-100 flex items-center justify-center">
            <ShoppingBag size={28} className="text-indigo-500" strokeWidth={2.2} />
          </div>
          <h1 className="mt-5 text-xl font-extrabold text-slate-800">Your cart is empty</h1>
          <p className="mt-2 text-sm text-slate-500 leading-relaxed">
            Add a product before checking out — we&apos;ll save your shipping details so next time is faster.
          </p>
          <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/products"
              className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-primary hover:bg-primary-dark text-white text-sm font-bold shadow-md shadow-primary/20 transition-colors">
              Browse products <ArrowRight size={14} />
            </Link>
            <Link href="/cart"
              className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 text-sm font-bold transition-colors">
              View cart
            </Link>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid lg:grid-cols-3 gap-7">
            ...
            <div className="lg:col-span-2 space-y-5">
                ...
                <div className="glass-card p-6 animate-fade-in-up">
                  <div className="flex items-center justify-between mb-5">
                    <h2 className="font-heading font-bold text-slate-900 flex items-center gap-3">
                      ...
                      <Home size={16} className="text-primary" /> Choose a saved address
                    </h2>
                    <span className="text-xs text-slate-400">{savedAddresses.length} saved</span>
                  </div>
                  <SavedAddressPicker
              ...
              <div className="glass-card p-6 animate-fade-in-up">
                <h2 className="font-heading font-bold text-slate-900 mb-5 flex items-center gap-3">
                  <span className="w-7 h-7 rounded-full bg-primary text-white text-xs font-extrabold flex items-center justify-center shrink-0">
                    {user && savedAddresses.length > 0 ? 2 : 1}
                  </span>
                ...
                <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="tel" value={recipientPhone} onChange={e => setPhone(e.target.value)}
                          ...
                          pattern="^(?:\+?977[-\s]?)?(?:9[678]\d{8})$"
                          title="Nepal mobile number (98XXXXXXXX, 97XXXXXXXX, or 96XXXXXXXX)"
                          className="w-full pl-10 pr-4 py-3.5 rounded-xl text-sm border border-white/80 text-slate-800 outline-none focus:border-primary focus:ring-2 focus:ring-primary/15 transition-all" style={{ background: 'rgba(255,255,255,0.75)', backdropFilter: 'blur(8px)' }} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 flex items-center gap-2 flex-wrap">
                      Email Address
                      <span className="text-[10px] text-slate-400 font-normal normal-case tracking-normal">
                        for order confirmation &amp; receipt
                      </span>
                      {!user && <LoginPromptLink />}
                    </label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        readOnly={!!user}
                        className={`w-full pl-10 pr-4 py-3.5 rounded-xl text-sm border border-white/80 text-slate-800 outline-none focus:border-primary focus:ring-2 focus:ring-primary/15 transition-all ${user ? 'cursor-not-allowed opacity-80' : ''}`}
              ...
              {/* ── Nepal Address — show only when adding new or no saved selected ─ */}
              {(!user || isAddingNew || savedAddresses.length === 0) && (
                <div className="glass-card p-6 animate-fade-in-up delay-100">
                  <h2 className="font-heading font-bold text-slate-900 mb-5 flex items-center gap-3">
                    <span className="w-7 h-7 rounded-full bg-primary text-white text-xs font-extrabold flex items-center justify-center shrink-0">
                      {user && savedAddresses.length > 0 ? 3 : 2}
                    </span>
              ...
              <div className="glass-card overflow-hidden">
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                  <h2 className="font-heading font-bold text-slate-900 flex items-center gap-3">
                    <span className="w-7 h-7 rounded-full bg-primary text-white text-xs font-extrabold flex items-center justify-center shrink-0">
                      {user && savedAddresses.length > 0 ? (isAddingNew ? 4 : 3) : 3}
                    </span>
              ...
              {!weatherLoading && weatherData && (
                <WeatherWidget store={weatherData.store} destination={weatherData.destination} storeLabel={weatherData.storeLabel} />
              )}
              ...
              {/* ── Guest: save my info for next time ─────────────────── */}
              {!user && (
                <SaveMyInfoCheckbox
                  checked={saveInfo}
            ...
            <div>
              <div className="glass-card p-6 sticky top-20">
                ...
                <div className="mb-4">
                  {giftCardApplied ? (
                    <div className="flex items-center gap-2 px-3 py-2.5 rounded-2xl bg-amber-50 border border-amber-200">
                      <Gift size={13} className="text-amber-600 shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-amber-700 truncate">{giftCard.toUpperCase()}</p>
                        <p className="text-[10px] text-amber-600">
                          {formatPrice(giftCardBalance)} available
                          {giftCardDiscount < giftCardBalance && giftCardDiscount > 0 && (
                            <span> · using {formatPrice(giftCardDiscount)} on this order</span>
                          )}
                        </p>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <div className="relative flex-1">
                          ...
                          <input
                            type="text"
                            value={giftCard}
                            onChange={e => { setGiftCard(e.target.value); setGiftCardError('') }}
                            onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), applyGiftCard())}
                            placeholder="Gift card code"
                            className="w-full pl-8 pr-3 py-2.5 rounded-xl text-sm border border-slate-200 bg-white/80 text-slate-800 outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-400/15 transition-all"
                          />
                        </div>
                        <button
                          type="button"
                          ...
                          className="px-4 py-2.5 text-sm font-bold bg-amber-500 text-white rounded-xl hover:bg-amber-600 transition-colors cursor-pointer shrink-0 disabled:opacity-60 flex items-center gap-1.5"
                        >
                          {giftCardValidating ? <><Loader2 size={13} className="animate-spin" /> Checking…</> : 'Apply'}
                        </button>
                      </div>
                      {giftCardError && (
                        <p className="text-[11px] text-red-500 font-medium px-1">{giftCardError}</p>
                      )}
                    </div>
                  )}
                </div>
                ...
                {!user && saveInfo && recipientEmail.trim() && (
                  <p className="mt-3 flex items-start gap-1.5 text-[11px] text-slate-500 leading-relaxed">
                    ...
                    <span>We&apos;ll email you a one-tap link to claim your account + 10% off your next order.</span>
                  </p>
                )}
              </div>
            </div>
